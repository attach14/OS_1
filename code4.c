...
int main(int argc, char* argv[]) {
  ...
  if (result1 == 0) { //первый канал читает и передаёт второму
    ...
    if (result2 == 0) { //второй процесс анализирует и передает третьему
      ...
      if (result3 > 0) { //второй процесс завершает работу
        return 0;
      }
      if (result3 == 0) { //третий процесс выводит текст из второго канала в файл
        int fd_write;
        ssize_t  read_bytes, res;
        read_bytes = read(fdTwo[0], buffer, size);
        if(read_bytes == -1){
          printf("Child 3: Can't read from pipe #2\n");
          return 0;
        }
        ...
        if(close(fdTwo[0]) < 0){
          printf("Child 3: Can't close reading side of pipe #2\n"); 
          return 0;
        }
        printf("OK\n");
        return 0;
